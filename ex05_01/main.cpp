...
struct TriMesh
{
  ...
  std::vector<Eigen::Vector3d> vertex_normals;  // Normals Each vertex has a normal -> allows for a clean display, and has two normals -> corners. Average of normals on adjacent faces
  std::vector<Eigen::Vector3d> vertex_colors;
  std::vector<Eigen::Vector2d> tex_coords;
  std::vector<Eigen::Vector3i> triangles;       // Remember the number of vertices, those that have connection information.
...
{
    for (int i = 0; i < io_Mesh.vertices.size(); i++)   // Shading is performed on each coordinate of the mesh structure.
    {
...
// draw mesh
void drawMesh(const TriMesh& in_Mesh)       // Receive mesh variables and enable textures
{
...
void generateTessellatedSphere(TriMesh& out_mesh, const Eigen::Vector3d& in_center, const double& in_r, const Eigen::Vector3d& in_kd, const int in_nSegs)
{
    ...
    for (k = 0; k < c + 1; k++) {
        ...
        TriMesh s;
        generateTessellatedSquare( s, in_p1+ in_center, in_p2 + in_center, in_p3 + in_center, in_kd, in_nSegs);
        extendMesh(out_mesh, s);
...
int main( int argc, char* argv[] )
{
  ...
  g_Texture = prepareTextureFromJpegFile( "/CGProgramming/ex05/Release/Lenna.jpg" );

  //ex5 make Sphere
  generateTessellatedSphere(g_Sphere, { 0.7, 0.3, -0.2 }, 0.3, { 0.0, 0.3, 0.0 }, 14);
